...
const { CREATE_ONLY_UNIT_SCHEMA } = require('../../../../../../models/v1/bookings/schema')

module.exports = handler(async ({ body, user: { accountId }, params: { id: propertyUnitTypeUnitId } }, res) => {
  const {
    dateArrival,
    dateDeparture,
    ...
    source,
    notes,
  } = await validate(body, { schema: CREATE_ONLY_UNIT_SCHEMA })
   ...
   */
  const property = await selectPropertyBy({ id: unit.propertyId, accountId })
  ...
  const propertyUnitTypeId = unitType.id

...
const { createSchema } = require('../../../../../../models/v1/booking-extras/schema')
...
module.exports = handler(async ({ body, params: { bookingId }, user: { accountId } }, res) => {
  ...
  const payload = await validate(body, {
    schema: createSchema(extrasType),
  })
  ...
  const id = await createTransaction(async (trx) => {
    const extrasId = await createBookingExtras(extrasType, {
      ...data, bookingId,
    }, trx)

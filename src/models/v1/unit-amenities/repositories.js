...
exports.upsertBy = async (propertyUnitTypeUnitId, data, trx) => {
  ...
  const amenities = await exports.selectBy({ propertyUnitTypeUnitId }, trx)
  ...
  return Promise.all([
    ...
    payload.filter((o) => amenities.find((amenity) => Number(amenity.dictAmenityId) === Number(o.dictAmenityId)))
      .map((params) => (
        update(TABLE_NAME, params, { propertyUnitTypeUnitId, dictAmenityId: params.dictAmenityId }, trx)
      )),

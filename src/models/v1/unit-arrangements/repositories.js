...
exports.upsertBy = async (propertyUnitTypeUnitId, data, trx) => {
  ...
  const arrangements = await exports.selectBy({ propertyUnitTypeUnitId }, trx)
  ...
  return Promise.all([
    ...
    payload.filter((o) => (
      ...
      update(TABLE_NAME, params, { propertyUnitTypeUnitId, dictArrangementId: params.dictArrangementId }, trx)
    )),
    ...
    ).map((params) => insert(TABLE_NAME, { ...params, propertyUnitTypeUnitId }, trx)),
  ].flat())
}

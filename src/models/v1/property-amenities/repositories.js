...
exports.upsertBy = async (propertyId, data, trx) => {
  ...
  const amenities = await exports.selectBy({ propertyId }, trx)
  ...
  return Promise.all([
    ...
    payload.filter((o) => amenities.find((amenity) => Number(amenity.dictAmenityId) === Number(o.dictAmenityId)))
      .map((params) => update(TABLE_NAME, params, { propertyId, dictAmenityId: params.dictAmenityId }, trx)),

    /**
     * Create new amenities
     */
    payload.filter((o) => !amenities.find((amenity) => Number(amenity.dictAmenityId) === Number(o.dictAmenityId)))
      .map((params) => insert(TABLE_NAME, { ...params, propertyId }, trx)),
  ].flat())
}

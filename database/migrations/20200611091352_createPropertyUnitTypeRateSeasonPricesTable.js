...
exports.up = (knex) => knex.schema.createTable(TABLE_NAME, (table) => {
  ...
  table.integer('property_unit_type_rate_season_id').unsigned().notNull()

  table.boolean('enabled').notNull().defaultTo(true)
  table.integer('accommodation').unsigned().notNull()

  table.boolean('price_default').notNull().defaultTo(false)
  ...
  table.foreign('property_unit_type_rate_season_id')
    .references('property_unit_type_rate_seasons.id')
    .onUpdate('CASCADE')
    .onDelete('CASCADE')
    .withKeyName('ut_rate_season_prices_ut_rate_season_price_id_foreign')
})
